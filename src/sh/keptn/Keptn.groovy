...
def keptnInit(Map args) {
    ...
    if (monitoring != "") {
        error("keptnInit no longer supports 'monitoring' argument. Use keptnConfigureMonitoring function after adding " +
                "all the relevant resources to the project/service/stages")
    }
...
def keptnConfigureMonitoring(Map args) {
    ...
    keptnEnrichedArgs = keptnLoadFromInit(args)

    String monitoring = keptnEnrichedArgs.containsKey("monitoring") ? args.monitoring : ""

    if (!monitoring?.trim()) {
        error("keptnConfigureMonitoring needs a 'monitoring' argument specifying the desired type of monitoring")
    }
